...
/*******Specular Occlusion*******/
//////////////////////////////////
half SpecularOcclusionFromBentAO(real3 V, real3 normalWS, real3 bentNormalWS, half ambientOcclusion, half roughness)
{
...
half SpecularOcclusionFromGI(VectorsData vectorsData, half3 bakedGI, half3 reflectVector, half perceptualRoughness, half ambientOcclusion, half giOcclusionBias)
{
...
half CalculateSpecularOcclusion(BRDFData brdfData, VectorsData vectorsData, half3 bakedGI, half3 reflectVector, half occlusion, half horizonFade, half giOcclusionBias)
{
    half specularOcclusion = 1.0;

#if defined(_AO_SPECULAR_OCCLUSION)
    specularOcclusion = saturate(PositivePow(saturate(dot(vectorsData.normalWS, vectorsData.viewDirectionWS)) + occlusion, exp2(-16.0 * brdfData.roughness - 1.0)) - 1.0 + occlusion);
#elif (defined(_BENTNORMALMAP) || defined(_BENTNORMALMAP1) || defined(_BENTNORMALMAP2) || defined(_BENTNORMALMAP3)) && defined(_BENTNORMAL_SPECULAR_OCCLUSION)
    specularOcclusion = SpecularOcclusionFromBentAO(vectorsData.viewDirectionWS, vectorsData.normalWS, vectorsData.bentNormalWS, occlusion, brdfData.roughness);
#elif defined(_GI_SPECULAR_OCCLUSION)
    specularOcclusion = SpecularOcclusionFromGI(vectorsData, bakedGI, reflectVector, brdfData.perceptualRoughness, occlusion, giOcclusionBias);
#endif

#ifdef _HORIZON_SPECULAR_OCCLUSION
    specularOcclusion *= GetHorizonOcclusion(vectorsData.viewDirectionWS, vectorsData.normalWS, vectorsData.geomNormalWS, horizonFade);
#endif
...
/**********SimpleLit GI**********/
//////////////////////////////////
half3 GlobalIllumination(BRDFData brdfData, half3 bakedGI, float3 positionWS, half3 geomNormalWS, half3 normalWS, half3 viewDirectionWS, half occlusion, half horizonFade)
{
    half3 reflectVector = reflect(-viewDirectionWS, normalWS);
    half NoV = saturate(dot(normalWS, viewDirectionWS));
    half fresnelTerm = Pow4(1.0 - NoV);

    half specularOcclusion = 1.0h;
#ifdef _HORIZON_SPECULAR_OCCLUSION
    specularOcclusion *= GetHorizonOcclusion(viewDirectionWS, normalWS, geomNormalWS, horizonFade);
#endif

    half3 indirectDiffuse = bakedGI * occlusion;
    half3 indirectSpecular = GlossyEnvironmentReflection(reflectVector, positionWS, brdfData.perceptualRoughness, specularOcclusion);
...
half3 GlobalIllumination(BRDFData brdfData, half3 bakedGI, half3 geomNormalWS, half3 normalWS, half3 viewDirectionWS, half occlusion, half horizonFade)
{
    half3 reflectVector = reflect(-viewDirectionWS, normalWS);
    half NoV = saturate(dot(normalWS, viewDirectionWS));
    half fresnelTerm = Pow4(1.0 - NoV);
...
/*************Lit GI*************/
//////////////////////////////////
half3 GlobalIllumination(BRDFData brdfData, BRDFData brdfDataClearCoat, VectorsData vectorsData, float3 positionWS, half3 bakedGI, half clearCoatMask, half occlusion, half horizonFade, half giOcclusionBias)
{
    half3 reflectVector = reflect(-vectorsData.viewDirectionWS, vectorsData.normalWS);
    half NoV = saturate(dot(vectorsData.normalWS, vectorsData.viewDirectionWS));
    half fresnelTerm = Pow4(1.0 - NoV);

    half specularOcclusion = CalculateSpecularOcclusion(brdfData, vectorsData, bakedGI, reflectVector, occlusion, horizonFade, giOcclusionBias);
    ...
    half3 coatReflectVector = reflectVector;
        ...
        coatReflectVector = reflect(-vectorsData.viewDirectionWS, vectorsData.coatNormalWS);
    #endif
    half3 coatIndirectSpecular = GlossyEnvironmentReflection(coatReflectVector, positionWS, brdfDataClearCoat.perceptualRoughness, specularOcclusion);
...
half3 GlobalIllumination(BRDFData brdfData, BRDFData brdfDataClearCoat, VectorsData vectorsData, half3 bakedGI, half clearCoatMask, half occlusion, half horizonFade, half giOcclusionBias)
{
    half3 reflectVector = reflect(-vectorsData.viewDirectionWS, vectorsData.normalWS);
    half NoV = saturate(dot(vectorsData.normalWS, vectorsData.viewDirectionWS));
    half fresnelTerm = Pow4(1.0 - NoV);
    ...
    half3 coatReflectVector = reflectVector;
        ...
        coatReflectVector = reflect(-vectorsData.viewDirectionWS, vectorsData.coatNormalWS);
    #endif
    half3 coatIndirectSpecular = GlossyEnvironmentReflection(coatReflectVector, brdfDataClearCoat.perceptualRoughness, specularOcclusion);
    // TODO: "grazing term" causes problems on full roughness
    half3 coatColor = EnvironmentBRDFClearCoat(brdfDataClearCoat, clearCoatMask, coatIndirectSpecular, fresnelTerm);
...
/**********ComplexLit GI**********/
///////////////////////////////////
half3 GlobalIllumination(BRDFData brdfData, BRDFData brdfDataClearCoat, BSDFData bsdfData, VectorsData vectorsData, 
                            float3 positionWS, half3 bakedGI, half occlusion, half horizonFade, half giOcclusionBias)
{
#if !defined(_MATERIAL_FEATURE_ANISOTROPY)
    half3 reflectVector = reflect(-vectorsData.viewDirectionWS, vectorsData.normalWS);
#else
    half3 modifiedNormalWS = vectorsData.normalWS;
    half3 bitangentWS = vectorsData.tangentWS.w * cross(vectorsData.normalWS, vectorsData.tangentWS.xyz);
    GetGGXAnisotropicModifiedNormalAndRoughness(bitangentWS, vectorsData.tangentWS.xyz, vectorsData.normalWS, vectorsData.viewDirectionWS, bsdfData.anisotropy, brdfData.perceptualRoughness, modifiedNormalWS, brdfData.perceptualRoughness);
    half3 reflectVector = reflect(-vectorsData.viewDirectionWS, modifiedNormalWS);
#endif
    half NoV = saturate(dot(vectorsData.normalWS, vectorsData.viewDirectionWS));
    half fresnelTerm = Pow4(1.0 - NoV);
    ...
    half3 color = EnvironmentBRDF(brdfData, indirectDiffuse, indirectSpecular, fresnelTerm);
#else
    half3 fresnelIridescent = bsdfData.iridescence;
    half3 color = EnvironmentBRDFIridescence(brdfData, indirectDiffuse, indirectSpecular, fresnelTerm, fresnelIridescent);
#endif
...
#if defined(_CLEARCOAT) || defined(_CLEARCOATMAP)
    #if defined (_MATERIAL_FEATURE_ANISOTROPY)
    half3 coatReflectVector = reflect(-vectorsData.viewDirectionWS, vectorsData.normalWS);
    #else
    half3 coatReflectVector = reflectVector;
    #endif
    #if defined (_CLEARCOAT_NORMALMAP)
    ...
    half3 coatIndirectSpecular = GlossyEnvironmentReflection(coatReflectVector, positionWS, brdfDataClearCoat.perceptualRoughness, specularOcclusion);
    // TODO: "grazing term" causes problems on full roughness
    half3 coatColor = EnvironmentBRDFClearCoat(brdfDataClearCoat, bsdfData.clearCoatMask, coatIndirectSpecular, fresnelTerm);

    // Blend with base layer using khronos glTF recommended way using NoV
    // Smooth surface & "ambiguous" lighting
    // NOTE: fresnelTerm (above) is pow4 instead of pow5, but should be ok as blend weight.
    half coatFresnel = kDielectricSpec.x + kDielectricSpec.a * fresnelTerm;
    return (color * (1.0 - coatFresnel * bsdfData.clearCoatMask) + coatColor) * occlusion;
#else
    return color * occlusion;
#endif
}
...
half3 GlobalIllumination(BRDFData brdfData, BRDFData brdfDataClearCoat, BSDFData bsdfData, VectorsData vectorsData, 
                            half3 bakedGI, half occlusion, half horizonFade, half giOcclusionBias)
{
#if !defined(_MATERIAL_FEATURE_ANISOTROPY)
    half3 reflectVector = reflect(-vectorsData.viewDirectionWS, vectorsData.normalWS);
#else
...
half3 GlobalIllumination(BRDFData brdfData, VectorsData vectorsData, half3 bakedGI, half occlusion, half horizonFade, half giOcclusionBias)
{
    const BRDFData noClearCoat = (BRDFData)0;
    return GlobalIllumination(brdfData, noClearCoat, vectorsData, bakedGI, 0.0, occlusion, horizonFade, giOcclusionBias);
}

// Backwards compatiblity
half3 GlobalIllumination(BRDFData brdfData, BSDFData bsdfData, VectorsData vectorsData, half3 bakedGI, half occlusion, half horizonFade, half giOcclusionBias)
{
    const BRDFData noClearCoat = (BRDFData)0;
    return GlobalIllumination(brdfData, noClearCoat, bsdfData, vectorsData, bakedGI, occlusion, horizonFade, giOcclusionBias);
}
