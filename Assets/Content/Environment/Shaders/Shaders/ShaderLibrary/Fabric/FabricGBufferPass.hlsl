...
#if defined(_NORMALMAP) || defined(_DOUBLESIDED_ON) || defined(_MATERIAL_FEATURE_ANISOTROPY)
...
void InitializeInputData(Varyings input, half3 normalTS, real4 tangentWS, out InputData inputData)
{
...
#if defined(_NORMALMAP) || defined(_THREADMAP) || defined(_DOUBLESIDED_ON)
...
FragmentOutput LitGBufferPassFragment(Varyings input, half faceSign : VFACE)
{
    ...
    //AO Fresnel
    surfaceData.occlusion = lerp(1.0, surfaceData.occlusion, saturate(dot(input.normalWS, input.viewDirWS)));
    ...
    tangentWS = input.tangentWS; 
#endif
    InitializeInputData(input, surfaceData.normalTS, tangentWS, inputData);
    ...
    half3 color = GlobalIllumination(brdfData, inputData.bakedGI, tangentWS, inputData.normalWS, inputData.viewDirectionWS, surfaceData.anisotropy, surfaceData.occlusion);

    return BRDFDataToGbuffer(brdfData, inputData, surfaceData.smoothness, color);
}
